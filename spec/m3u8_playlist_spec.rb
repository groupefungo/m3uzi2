...
describe MediaPlaylist do
  ...
  it 'has a specification' do
    expect(MediaPlaylist.specification).to be_kind_of MediaPlaylistSpecification
  end

  it 'returns true or false if a tag name is valid' do
    expect(MediaPlaylist.valid_tag?('EXTINF')).to be true
    expect(MediaPlaylist.valid_tag?('EXTIVALIDTAG')).to be false
  end

  it 'creates a media segment' do
    expect(MediaPlaylist.create_media_segment('2013-08-091212.ts')).to \
